...
class MentorContactForm(forms.ModelForm):
    ...
    def clean_mentor_name(self):
        ...
        if mentor_name and not MentorProfile.objects.filter(name__iexact=mentor_name).exists():
            self.fields['mentor_name'].widget.attrs.update({'class': 'form-control is-invalid'})
            raise forms.ValidationError("This mentor doesn't exist. Leave blank to find any mentor.")
