...
from timeline.models import Honour, Timeline


def get_timeline_events():
    """Retrieve all timeline events with honour count"""
    return Timeline.objects.annotate(honour_count=Count('honours')).order_by(
        'year'
    )
...
def mark_user_honoured_events(events, user):
    ...
    if (
        ...
        and hasattr(user, 'honours')
        and hasattr(user.honours, 'values_list')
    ):
...
def toggle_event_honour(timeline, user):
    ...
    honour, created = Honour.objects.get_or_create(
    ...
    return {'honoured': honoured, 'honour_count': honour_count}
